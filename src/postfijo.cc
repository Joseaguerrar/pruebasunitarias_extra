...
Postfijo::Postfijo(std::queue<Token*> cola)
    : cola(cola), error(""), valid(true), resultado(0) {
    Evaluar();
}

std::string Postfijo::Error() const { return error; }

bool Postfijo::IsValid() const { return valid; }

double Postfijo::Resultado() const { return resultado; }
